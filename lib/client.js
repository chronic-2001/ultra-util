...
const {UltraStatus, ContentHandler} = require('./constants');
...
class Client {
  ...
  async createCourse(course) {
    course = await this.rest.post(COURSE_URL, { body: course });
    // need the following request to get all course folders prepared to allow uploading files
    await this.request.get(BASE_URL + '/webapps/blackboard/execute/courseMain?course_id=' + course.id);
    return course;
  }
  ...
  async createAssignment(courseId, assignment, files) {
    const contentUrl = `${COURSE_URL}/${courseId}/contents`;
    let res = await this.rest.get(contentUrl, { qs: { q: 'Content', contentHandler: ContentHandler.FOLDER, fields: 'id' } });
    const parentId = res.results[0].id;
    ...
    res = await this.rest.get(`${contentUrl}/${parentId}/children`, { qs: { q: assignment.contentName, contentHandler: ContentHandler.ASSIGNMENT, fields: 'id' } }); 
    return { id: res.results[0].id, ...assignment };
  }
